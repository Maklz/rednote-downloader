...
- `TELEGRAM_TARGET_CHAT_ID`: 可选，解析结果发布到的频道或会话。设置后媒体发到该频道，
  发送者只收到确认消息。机器人需要是该频道的管理员。
- `ANTHROPIC_API_KEY` / `TRANSLATION_ENABLED`: 可选，两者都设置后，发布到频道的文案会先翻译成俄语。
  翻译失败时回退为原文，不会丢帖。按调用计费。
